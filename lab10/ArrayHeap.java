...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private int size;
    private Map<T, Integer> nodeToIndex;
    ...
    private void swapNodeMapping(T n1, T n2){
        int temp = nodeToIndex.get(n1);
        nodeToIndex.put(n1, nodeToIndex.get(n2));
        nodeToIndex.put(n2, temp);
    }
    ...
    private void swim(int index) {
        ...
        while (inBounds(parentIndex(index))
                && contents[index].myPriority < contents[parentIndex(index)].myPriority) {
            if (!nodeToIndex.isEmpty())
                swapNodeMapping(getNode(index).item(), getNode(parentIndex(index)).item());
            swap(index, parentIndex(index));
            index = parentIndex(index);
        }
    }

    private void sinkHelp(int index){
        if (!inBounds(leftIndex(index))) {
            ...
            if (contents[index].myPriority > contents[leftIndex(index)].myPriority) {
                if (!nodeToIndex.isEmpty())
                    swapNodeMapping(getNode(index).item(), getNode(leftIndex(index)).item());
                swap(index, leftIndex(index));
                sinkHelp(leftIndex(index));
            }
        }
        else {
            int minIndex = contents[leftIndex(index)].myPriority
                    < contents[rightIndex(index)].myPriority
                    ? leftIndex(index) : rightIndex(index);
            if (!nodeToIndex.isEmpty())
                swapNodeMapping(getNode(index).item(), getNode(minIndex).item());
            swap(index, minIndex);
            sinkHelp(minIndex);
        }
    }
    ...
    private T delete(T item) {
        if (!nodeToIndex.containsKey(item)) {
            return null;
        }
    ...
    public void testDelete(){
        ...
        pq.insert("e", 5);
        pq.insert("b", 2);
        String removed = pq.delete("i");
        assertEquals("i", removed);
        assertEquals("g", pq.delete("g"));
        assertEquals("h", pq.delete("h"));
        assertEquals("e", pq.delete("e"));
        assertEquals("c", pq.delete("c"));
